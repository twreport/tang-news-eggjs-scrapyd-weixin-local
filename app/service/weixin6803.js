...
class Weixin6803Service extends Service {
    async start() {
        console.log('Start Spide 1!');
        // 查看uin的情况，如果能够爬取再爬取
        const result = await this.service.db.can_i_crawl();
        if(result === true){
            await this.weixin6803Driver();
        }else{
    ...
    // 定时执行scrapy任务
    async weixin6803Driver() {
        const scrapyd_server_url = this.app.config.ScrapydServerUrl6803;
        // 拼接curl地址
...
module.exports = Weixin6803Service;